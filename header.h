...
typedef struct {
    int type_error; //tipo di errore (es. nome non valido, tipo non valido, variabile non utilizzata)
    int num_line;
} Error;
...
ListType* create_ListType();           // Crea una lista vuota  (per ogni tipo nuovo chiamo questa funzione)
void add_type(ListType *list, char *type);  // Aggiunge un tipo alla lista 
int flag_type_exists(ListType *list, char *type);     // Controlla se un tipo esiste già (flag 1 o 0) tramite ricerca volendo binaria
