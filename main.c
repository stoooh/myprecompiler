...
int main(int argc,
         char *argv[]) { // args è il numero di argomenti, e argv l'array
  ...
  if (par == NULL || par->file_input == NULL) { // la freccia è un puntatore
    fprintf(
        stderr,
        "Errore: parametro -i obbligatorio!\n"); // stampa l'errore su schermo
    fprintf(stderr,
            "Uso appropriato: %s -i <file_input> [-o <file_output>] [-v]\n",
  ...
  ListType *custom_types = create_ListType(); // creo la lista dei typedef
